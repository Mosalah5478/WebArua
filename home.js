// Year
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.querySelectorAll('.nav-link').forEach(link => {
        link.addEventListener('click', function(e) {
          // Skip for same page links (anchors)
    ...
    // Enhanced scroll animations with AOS
    // The original reveal on scroll is kept for browsers that might not support AOS
    const io = new IntersectionObserver((entries)=>{
      entries.forEach(el=>{
        if(el.isIntersecting && !el.target.hasAttribute('data-aos')){
    ...
    document.querySelectorAll('.section .wrap, .hero .wrap').forEach(el=>{
      if(!el.hasAttribute('data-aos')) io.observe(el);
    });
    
    // Refresh AOS on scroll for better animations
    window.addEventListener('scroll', function() {
      AOS.refresh();
    });
    
    // Add scroll direction detection for enhanced animations
    let lastScrollTop = 0;
    window.addEventListener('scroll', function() {
